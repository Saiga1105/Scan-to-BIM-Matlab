function [mx, my,inlierNum, error,xy] = F_Linefit_RANSAC3(Cx,Cy,Px,Py,k,threshDist,dw,n)
...
% 	ptSample = pts(:,sampleIdx)';
    ...
    % recompute model
    modelLeastSquares = polyfit(Cx(inlierIdx),Cy(inlierIdx),1);
    my(:,i) = modelLeastSquares(1)*mx + modelLeastSquares(2);
    
    % compute error with repsect to P
    mapxy= [Px Py];
    [xy,distance,~] = distance2curve(curvexy,mapxy,'linear');
    error(i)=mean(abs(abs(distance)-dw/2));
