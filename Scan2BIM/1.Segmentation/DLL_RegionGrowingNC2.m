function cluster = G_RegionGrowingNC2( points,normals,colors,ThresValN,ThresValC,MaxDist,Minsize,Offset, Tilesize )
%% Explenation
...
%pctotal=pointCloud(points,'Color',colors,'Normal',normals);

%% construct point cloud
pctotal=pointCloud(points);
color_criteria=1;
if ~exist('normals', 'var') || isempty(normals)
    normals = pcnormals(pctotal); %10cm default
    if ~exist('colors', 'var') || isempty(colors)
        color_criteria=0;
        pctotal=pointCloud(points,'Normal',normals);
    else
        pctotal=pointCloud(points,'Normal',normals,'Color',colors);
    end 
else
    if ~exist('colors', 'var') || isempty(colors)
        color_criteria=0;
